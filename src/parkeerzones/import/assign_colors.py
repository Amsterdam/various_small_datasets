...
all_colors = {'#009DEC','#00A03C','#BED200','#FF9100','#E50082','#A00078','#EC0000','#767676'}
...
WHERE A.id = %s AND B.color IS NOT NULL
            ...
            if is_group:
                curs.execute(f"UPDATE {table_name} SET color = %s WHERE parent = %s", (new_color, id))
            else:
                curs.execute(f"UPDATE {table_name} SET color = %s WHERE ogc_fid = %s", (new_color, id))
